...
| orderby `{orderbyClause}`(, `{orderbyClause}`)* </br> where</br>`{orderbyClause}` is an content of `orderby` clause</br><pre>{orderbyClause}`:=`{expression1OfElementInQuerableBean}` (ascending\|descending)?</br></pre>|  | | | | 
